...
#Discord Command
prefix = "$"
TOKEN = "OTkyOTY0ODY4ODQ2MTI5MjYy.GxYMAz.3y6LoGAwxu88Vg9cxapOGCtLoYttrSGFAAUEZU"
bot = commands.Bot(command_prefix=prefix)
bot.remove_command('help')

#Calls
bot_developer = 'Phiso#2899 | Axvry'

#color
color = 0x8317FF

#Admin List - Insert the user id of the person here 
admin_list = [946205507998326815]
...
async def on_ready():
    ...
    conn.commit()
    print("added")
    await bot.change_presence(activity=discord.Game(name=f"Watching Axvry Key & Gen"))
...
async def info(ctx):
    ...
    role = guild.get_role(973042618990727210)
    size = len(role.members)
    embed=discord.Embed(title="Info",color=color,inline=True)
    embed.set_thumbnail(url=ctx.bot.user.avatar_url)
    embed.add_field(name="Premium:",value=size,inline=True)
...
async def generatekey(ctx,length,id:int = None):
    if ctx.author.id in admin_list:
                if id == None:
                    ...
                    await ctx.send(embed=embed)
                    try:
                        server = bot.get_guild(973042618990727209)
...
async def add_day(ctx, member:discord.Member, amount:int):
    if ctx.message.author.id in admin_list:
        ...
        await ctx.send(embed=embed)
@bot.command()
@commands.has_role("Premium Access")
        ...
        try:
            ...
            for i in range(expired_user):
                try:
                    ...
                    user_id = (int(user))
                    print(user)
                    guild = bot.get_guild(973042618990727209)
                    member = guild.get_member(973042618990727210)
                    role = guild.get_role(946205507998326815)
                    await member.remove_roles(role)
                    embed=discord.Embed(title="Thank you for using us!",description=f"Hey {member.name},\n\nThank you for purchasing our service. Your license has now expired and your role has been removed. If you wish to extend your license, please dm Phiso#2899 to buy",color=color)
                    embed.set_thumbnail(url=ctx.bot.user.avatar_url)
